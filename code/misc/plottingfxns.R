...
## FUNCTIONS FOR PLOTTING CHANGE IN PATHOLOGY LEVELS ACROSS TIME
plot_format <- function(df){
  ...
  df$path[df$path == -Inf] <- -4 # replace all -Inf with -4 for visualization purposes
  df <- df %>%
    filter(!is.na(path) & !is.infinite(path)) # filter out NA regions
  return(df)
}

ord_tps <- function(mpi) {
  ord_tp = case_when(mpi == 0.1 ~ 1,
          mpi == 0.2 ~ 2,
          mpi == 0.3 ~ 3,
          mpi == 0.5 ~ 4,
          mpi == 1 ~ 5,
...
tp_max <- function(peak_tp, max_path, path_df) {
  regions_peak <- max_path %>%
    filter(time_at_max == peak_tp)
  max_regions <- regions_peak$region
  peak <- path_df[path_df$region %in% max_regions, ]
  peak$peak_tp <- peak_tp
...
p.xy <- function(x,y,xlab,ylab,ttl='',col='black',alpha=1,sm.method = 'lm',formula=NULL,p.force=NULL,r.method='pearson'){
  ...
  p <- ggplot(df) + geom_point(aes(x=x,y=y),color=col,stroke=0,alpha=alpha) + 
    ...
    scale_y_continuous(limits = c(-6, 0)) + ## 5/31/24 added this to make y axis the same across all plots

### This script generates a plot that shows the log total pathology, cell body pathology, and neurite pathology summed across all ipsilateral 
...
# load data for neurite, cell body and total path; pull out ipsilateral regions
...
total_cb_path <- cb_path %>%
  select(starts_with("i")) %>%
  mutate(cb_total = rowSums(., na.rm=TRUE)) # sum across all regions to get total pathology for each mouse
load(paste0(opdir,'processed/total_pathdata.RData'))  # load total path data and ROI names
total_path <- path.data
total_path <- total_path %>%
  select(starts_with("i")) %>%
  mutate(total = rowSums(., na.rm=TRUE)) # sum across all regions to get total pathology for each mouse
...
ggsave(t,filename = paste0(savedir,'ipsi_neurite_v_cell_body_v_total_progression.pdf'),
